...
function extract(str, start, end, func, repl) {
	...
	*/
	while (str.search(start) > -1) 
	{
		s = str.search(start);
		e = str.indexOf(end, s); 
		if (e == -1 ) {e = str.length; /* 找不到"end", 將e設定為字串尾 */}
		/* 
			[ 0, s )
			[ s, e+(end.length) )
			[ e+(end.length), str.length )
		*/
		if (repl) 
		{
			a.push(func(str.substring(s, e + (end.length)))); 
			/* 將 [s, e+(end.length)] 的字以 func 方式上色, 並放入 a[] 中*/
			str = str.substring(0, s) + repl + str.substr(e + (end.length));
			/* 將 [s, e+(end.length)] 的字以 repl 暫時替代*/
		} 
		else /* angularMode */
		{
			d += str.substring(0, s);
			d += func(str.substring(s, e + (end.length)));
			/* 將 [s, e+(end.length)] 的字以 func 方式上色, 並直接放回 d 中*/
		...
		/* 
			...
			search(RegExp) position of the string, OR -1 if no string
			indexOf(s) first position of the string, OR -1 if no string
			lastIndexOf(s) last position of the string
			include(s) true/false
		*/
	}
	this.rest = d + str;
	this.arr = a;
...
function htmlMode(txt) {
	...
	while (rest.indexOf("&lt;") > -1) 
	{
		/* 
			rest: 全部文字/未處理的字串
			startpos/endpos: 此種標記的開頭結尾
			note: 標註 css || javascript
			done: 完成部分
		*/
		note = "";
